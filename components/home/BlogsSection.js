...
export default function BlogsSection() {
  ...
  useEffect(() => {
    const fetchRecentBlogs = async () => {
      try {
        const data = await blogService.getAll(1, 4); // Fetch 4 recent blogs
        setBlogs(data.blogs || []);
      } catch (error) {
        console.error("Failed to fetch blogs for home section", error);
      } finally {
        setLoading(false);
      }
    };
              ...
              <div className="pt-8 flex-1 flex flex-col justify-between">
                <div>
                   <div className="flex items-center gap-6 text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-4">
                      <div className="flex items-center gap-2"><Calendar size={12} className="text-[#E32219]" /> {featuredBlog.publishedDate ? format(new Date(featuredBlog.publishedDate), 'MMM dd, yyyy') : 'Recently'}</div>
                      <div className="flex items-center gap-2"><User size={12} className="text-[#E32219]" /> By {featuredBlog.author || 'Tridev Team'}</div>
                   </div>
