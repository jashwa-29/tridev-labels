  ...
  return (
    <section ref={sectionRef} className="py-24 md:py-36 bg-gray-50 overflow-hidden relative">
      <div className="container mx-auto px-6 md:px-12 relative z-10">
        <div className="max-w-4xl mb-20 capability-card" style={{ opacity: 0 }}>
          <div className="flex items-center gap-3 mb-6">
            <span className="w-10 h-px bg-[#E32219]"></span>
            <span className="text-[10px] font-bold uppercase tracking-[0.4em] text-gray-400">Technical Prowess</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {capabilities.map((item, i) => (
            <div key={i} className="capability-card group p-12 bg-white rounded-[40px] border border-gray-100 hover:border-[#E32219]/30 transition-all duration-700 shadow-sm hover:shadow-2xl" style={{ opacity: 0 }}>
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-4 block">/ {item.tag}</span>
      ...
      {/* Ghost Text */}
      <div className="absolute top-1/2 right-0 -translate-y-1/2 text-[20vw] font-black text-gray-100 select-none pointer-events-none -z-0 opacity-40">
        TECH
      </div>
    </section>
  );
